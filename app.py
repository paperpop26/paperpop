...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        def file_row(parent, label, var, cmd):
            ...
            tk.Entry(row, textvariable=var, font=("맑은 고딕",9), state="readonly", readonlybackground=LBLUE, relief="flat", bd=1).pack(side="left", fill="x", expand=True, padx=(0,6))
            tk.Button(row, text="찾기", command=cmd, bg=BLUE, fg=BTN_FG, font=("맑은 고딕",9), relief="flat", padx=8, cursor="hand2").pack(side="right")

        hdr_frame = tk.Frame(self, bg=BLUE, height=54)
        hdr_frame.pack(fill="x"); hdr_frame.pack_propagate(False)
        tk.Label(hdr_frame, text="🐸  페이퍼팝 송장 전송 자동화", bg=BLUE, fg=BTN_FG, font=("맑은 고딕",13,"bold")).pack(side="left", padx=16)
        tk.Label(hdr_frame, text=datetime.today().strftime("%Y-%m-%d"), bg=BLUE, fg="#A5C8F0", font=("맑은 고딕",10)).pack(side="right", padx=16)
        ...
        self.prev_listbox = tk.Listbox(lbf, font=("맑은 고딕",9), height=3, bg=LBLUE, relief="flat", bd=0, selectbackground=BLUE, selectforeground="white")
        ...
        tk.Button(br, text="+ 추가", command=self._add_prev, bg=BLUE, fg=BTN_FG, font=("맑은 고딕",9), relief="flat", padx=10, cursor="hand2").pack(side="left", padx=(0,6))
        tk.Button(br, text="삭제", command=self._del_prev, bg="#9CA3AF", fg=BTN_FG, font=("맑은 고딕",9), relief="flat", padx=10, cursor="hand2").pack(side="left")
        ...
        self.run_btn = tk.Button(run_frame, text="▶  송장 파일 만들기", command=self._run, bg=GREEN, fg=BTN_FG, font=("맑은 고딕",12,"bold"), relief="flat", pady=10, cursor="hand2")
        self.run_btn.pack(fill="x")

        s4 = section(self, "  📋  실행 로그")
        self.log_text = tk.Text(s4, height=10, font=("맑은 고딕",9), bg="#F9FAFB", relief="flat", bd=0, state="disabled", wrap="word")
    ...
    def _add_prev(self):
        paths = filedialog.askopenfilenames(title="미발송 운송장 파일 선택", filetypes=[("Excel 파일","*.xlsx")])
        for p in paths:
